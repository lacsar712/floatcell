...
import (
	...
	"github.com/lacsar712/floatcell/internal/airsparge"
	"github.com/lacsar712/floatcell/internal/flotcell"
	"github.com/lacsar712/floatcell/internal/fsm"
...
type App struct {
	...
	plant        *flotcell.CellPlant
	towerFSM     *fsm.TowerFSM
	zones        *flotcell.ZoneTable
	...
	airsparge      *airsparge.Controller
	holdMgr      *pulplevel.HoldWindowManager
	holdEval     *pulplevel.HoldWindowEvaluator
	gradAudit    *pulplevel.OrderedGradientValidator
	guard        *interlock.Guard
	dryFSM       *fsm.FloatFSM
	scheduler    *clock.SegmentScheduler
	avgWindow    *clock.AerateWindow
...
func New(cfg config.Config) (*App, error) {
	...
	zones, err := flotcell.NewZoneTable(towerID, cfg.ZoneCount, plenumID, cfg)
	if err != nil {
		return nil, err
	}
	plant := flotcell.NewCellPlant(cfg, clk, mem)
	...
	for i, z := range zones.Zones() {
		...
		plant.RegisterSensor(pulplevel.NewSensor(sensorID, z.Zone))
		targets = append(targets, cfg.TargetMoistPct)
	}
	if err := plant.InitProfile(zoneIDs, targets); err != nil {
		return nil, err
	}
	ventBank, err := airsparge.NewVentBank(zoneIDs, "vent-damper")
	if err != nil {
		return nil, err
	}
	...
	holdMgr := pulplevel.NewHoldWindowManager()
	a := &App{
		...
		gradAudit:    pulplevel.NewOrderedGradientValidator(zoneIDs, cfg.MaxGradientDeltaPct, 0.5),
		guard:        interlock.NewGuard(guardPairs),
		batchCancels: make(map[model.TowerID]context.CancelFunc),
	}
	a.airsparge = initDehumidController(ventBank, cfg.MaxGradientDeltaPct, func(readings []model.MoistureReading) float64 {
	...
	if pc, ok := a.clk.(*clock.ProcessClock); ok {
		...
		a.avgWindow = clock.NewAerateWindow(a.clk, time.Duration(cfg.AerateWindowMinutes)*time.Minute)
	}
	a.dryFSM = fsm.NewFloatFSM(towerID, a.onDryTransition)
	fsm.RegisterDryHeatHook(a.dryFSM.Hooks())
	a.dryRamp = NewFloatRamp(a.clk, cfg.ProcessTick(), cfg.FloatRampSteps)
