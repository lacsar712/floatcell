...
func (a *App) observeZoneMoisture(ctx context.Context) error {
	...
	if err := a.airsparge.ApplyCycle(ctx, readings); err != nil {
		return err
	}
	return a.airsparge.ValidateGradient(readings)
}
...
func (a *App) routeSummaryLine(plenum model.PlenumID) string {
	...
	return fmt.Sprintf("routes=%d active_vents=%d fan_load=%.0f",
		summary.RouteCount, a.airsparge.Bank().ActiveCount(), a.stager.CurrentLoad())
}
...
func initHoldEvaluator(profile *pulplevel.ProfileManager, windows *pulplevel.HoldWindowManager) *pulplevel.HoldWindowEvaluator {
	return pulplevel.NewHoldWindowEvaluator(profile, windows)
}

func initDehumidController(bank *airsparge.VentBank, maxDelta float64, gradientFn func([]model.MoistureReading) float64) *airsparge.Controller {
	c := airsparge.NewController(bank, maxDelta)
