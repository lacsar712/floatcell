...
type FloatFSM struct {
	id    model.TowerID
	state model.DryState
	hooks *DryHookChain
}

func NewFloatFSM(id model.TowerID, effect func(context.Context, model.TowerID, model.DryState, model.DryState) error) *FloatFSM {
	_ = effect
	return &FloatFSM{id: id, state: model.DryIdle, hooks: NewDryHookChain()}
}

func (f *FloatFSM) Hooks() *DryHookChain { return f.hooks }

func (f *FloatFSM) State() model.DryState { return f.state }

func (f *FloatFSM) Dispatch(ctx context.Context, event string) (model.DryState, error) {
